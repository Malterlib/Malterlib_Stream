...
namespace NMib::NStream
{
	...
	void CBinaryStorage::f_AppendRangeOf(CBinaryStorage const &_Source, umint _Offset, umint _nBytes)
	{
		...
		// The whole range against the limit before any of it is appended: the segments go in one
		// at a time, and one refused halfway would leave the earlier ones in place
		fp_CheckTotalLengthAdd(_nBytes);
