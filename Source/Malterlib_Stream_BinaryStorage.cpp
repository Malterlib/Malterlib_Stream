...
namespace NMib::NStream
{
	...
	// The tail may point into the source's inline segment and must be retargeted after a move.
	...
	// Counts top-level segments without recursing into nested storages.
	...
	// Shared and nested segments can grow the logical length without allocating. Check before mutating
	// so a failed append cannot leave bytes excluded from the recorded length.
	void CBinaryStorage::fp_CheckTotalLengthAdd(umint _nBytes) const
	{
		...
		// Stream lengths use signed CFilePos; reject lengths that would become negative.
	...
	// The removed range must lie wholly in the trailing local arena segment.
	...
	// Shared ranges retain their owners; arena and adopted bytes are copied. Nested storages are traversed recursively.
	void CBinaryStorage::f_AppendRangeOf(CBinaryStorage const &_Source, umint _Offset, umint _nBytes)
	{
		...
		// Validate the whole append before mutating; a partial append must not survive a length-limit error.
		fp_CheckTotalLengthAdd(_nBytes);

		umint RangeEnd = _Offset + _nBytes;
		umint Position = 0;

		// Arena and adopted bytes lack a shareable owner and must be copied.
	...
	// Returns a retaining view only when the range lies wholly in one shared segment.
	...
	// The range must lie wholly in one local arena segment. The pointer is valid only until the next storage mutation.
