...
namespace NMib::NStream
{
	...
	void TCBinaryStreamStorage<t_CStreamType>::f_FeedBytes(const void *_pMem, umint _nBytes)
	{
		...
		// Subtraction form so a position plus length that wraps cannot pass as in range
	...
	{
		// Write only by contract: reading a field back here would walk the segment list per
		// call. Anything that needs to read the content opens a TCBinaryStreamStoragePtr
		// over the storage, whose span table makes sequential reads cheap
...
namespace NMib::NStream
{
	namespace NPrivate
	{
		// Detects streams that can adopt a whole storage by reference (segmented storage
		// write streams)
		...
		// Detects streams that can hand out consumed bytes as a storage referencing their
		// backing shared content instead of copying it
		template <typename t_CStream, typename t_CEnableIf = void>
	...
	// can use CBinaryStorage, CSharedByteVector or CIOByteVector interchangeably, and
	// storage capable streams carry the content by reference in both directions
	template <typename t_CStream>
	...
	public:
		static void fs_Feed(t_CStream &_Stream, CBinaryStorage const &_Data)
		{
			...
			// A const feed still shares what is shareable: the storage's shared spans go in
			// by reference — variant and optional wrappers feed their members as const, so
			// this is the path a wrapped storage takes — and only arena or adopted vector
			// content, which has no owner to share, is copied
