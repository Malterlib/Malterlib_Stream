	...
	namespace NStream
	{
		...
		public:
			...
			void f_FeedBytes(const void *_pMem, mint _nBytes)
			{
				...
				if (CurrentLen < NeededSize)
					fp_GrowBufferGrow(NeededSize);
				NMem::fg_MemCopy(m_pBuffer + Position, _pMem, _nBytes);
				Position += _nBytes;
			...
			{ 
				if (_Length > NStream::CFilePos(m_Length))
				{
					...
					NMem::fg_MemClear(m_pBuffer + m_Length, _Length - m_Length);
				}
				m_Length = _Length;
			}
		};	
...
//					DMibError("Position is past end of stream");
		...
		public:
			...
			{
				fp_GrowBuffer(_nBytes);
				NMem::fg_MemCopy(m_Buffer.f_GetArray() + m_Position, _pMem, _nBytes);
				m_Position += _nBytes;
			...
			{ 
				...
				{
					fp_GrowBuffer(_Length - m_Length);
					NMem::fg_MemClear(m_Buffer.f_GetArray() + m_Length, _Length - m_Length);
				}
				m_Length = _Length;
			}
		...
		protected:
			...
			NContainer::TCVector<uint8> m_TempVector;
		...
		public:
			...
			void f_FeedBytes(const void *_pMem, mint _nBytes)
			{
				...
				if (NMib::NMem::fg_MemCmp((uint8 const *)_pMem, m_TempVector.f_GetArray(), _nBytes) != 0)
					DMibPDebugBreak;
