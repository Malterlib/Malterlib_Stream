		...
		void f_DoTests()
		{
			...
			DMibTestSuite("NestedStorage")
			{
...

				{
					...
					// Nesting shares the storage with readers, so it is frozen once built
			...
			{
				{
					...
					// The write stream is write only; reading goes through a read stream over
					// the storage
...

				{
					...
					// Patch the dummy packet id in place, like the packet queue does
			...
			{
				// The correctness anchor: the same serialization sequence must produce byte
				// identical output through the contiguous memory stream and the segmented
				// storage stream
			...
			{
				{
					...
					// The length prefix lands in the arena, the payload is adopted in place
					bool bAdopted = false;
...

				{
					...
					// The caller still holds its reference and the stream references the same bytes
